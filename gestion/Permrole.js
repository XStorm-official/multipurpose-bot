...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        if (owner.get(`owners.${message.author.id}`) || message.member.roles.cache.has(pgp.fetch(`permgp_${message.guild.id}`)) || message.member.roles.cache.has(perm) || config.app.owners.includes(message.author.id) === true) {
